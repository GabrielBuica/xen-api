...
module Export = struct
    ...
    module Json = struct
      module Zipkinv2 = struct
        ...
        let zipkin_span_of_span (s : Span.t) : ZipkinSpan.t =
          ...
          {
            id= SpanContext.span_id_of_span_context span_context
          ; traceId= SpanContext.trace_id_of_span_context span_context
          ; parentId=
              Option.map
                (fun x ->
          ...
          ; kind=
              Option.map SpanKind.to_string
                (ZipkinSpan.kind_to_zipkin_kind (Span.get_span_kind s))
          ; localEndpoint= {serviceName}
          ; annotations
          ; tags=
              Attributes.fold
                (fun k v tags -> (k, v) :: tags)
                (Span.get_attributes s) []
          }
  ...
  module Destination = struct
    ...
    let flush_spans () =
      ...
      |> List.filter (fun x -> x |> TracerProvider.get_enabled)
