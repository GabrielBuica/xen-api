...
module Span = struct
  type t = {
    ...
    ; attributes: string Tracing_attributes.t
  }
  ...
  let start ?(attributes = Tracing_attributes.empty) ~name ~parent ~span_kind ()
      =
    let trace_id =
      match parent with
      | None ->
  ...
  let get_tag t tag = Tracing_attributes.find tag t.attributes
  ...
  let get_attributes span =
    Tracing_attributes.fold (fun k v tags -> (k, v) :: tags) span.attributes []

  let finish ?(attributes = Tracing_attributes.empty) ~span () =
    let attributes =
      Tracing_attributes.union
        (fun _k a _b -> Some a)
  ...
  let add_event span name attributes =
    let attributes = Tracing_attributes.of_list attributes in
        ...
        | Unset ->
            let attributes =
              Tracing_attributes.union
                (fun _k a _b -> Some a)
                span.attributes
                (Tracing_attributes.of_list exn_attributes)
            in
...
module TracerProvider = struct
  ...
  let get_attributes t = Tracing_attributes.to_assoc_list t.attributes
  ...
  Xapi_stdext_threads.Threadext.Mutex.execute lock (fun () ->
      let provider =
        ...
        | Some (provider : TracerProvider.t) ->
            ...
            let attributes : string Tracing_attributes.t =
              Option.fold ~none:provider.attributes
                ~some:Tracing_attributes.of_list attributes
            in
            let endpoints =
