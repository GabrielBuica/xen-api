...
module Runtime = struct
  ...
  let maybe_thread_yield () =
    let thread_ctx =
      Xapi_stdext_threads.Threadext.ThreadLocalStorage.get () |> Option.get
    in
    (* Fun.flip Option.iter thread_ctx @@ fun thread_ctx -> *)
    (* TODO: Check the correct behavior: get did not return an option *)
    ...
    | Some tgroup ->
        ( if Mtime.is_later tgroup.epoch ~than:thread_ctx.tepoch then (
            ...
            thread_ctx.time_running <- Mtime.Span.zero ;
            thread_ctx.tepoch <- tgroup.epoch
          ) else
        ...
        *)
        ...
        if Mtime.Span.compare thread_ctx.time_running tgroup.time_ideal > 0 then (
          thread_ctx.time_last_yield <- Mtime_clock.counter () ;
  ...
  let sched_global_slice global_slice_time =
    ...
    let time_ideal_of_tgroups groups =
      Tgroup.ThreadGroup.(
        ...
        |> List.iter (fun g ->
               ...
               let group_time_ns =
                 group_share_ratio
                 *. (global_slice_time |> Atomic.get |> Mtime.Span.to_float_ns)
               in
