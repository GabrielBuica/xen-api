...
let test_maybe_thread_yield () =
  Sys.opaque_identity
    (Xapi_timeslice.Timeslice.Runtime.maybe_thread_yield
       ~global_slice_period:Mtime.Span.(10 * ms)
    )
...
let test_with_thread_classified ~name f =
  let allocate () =
    ...
    Xapi_stdext_threads.Threadext.ThreadRuntimeContext.(let thread_ctx = get ()
  in update (fun thread_ctx -> {thread_ctx with
      tgroup=Tgroup.Group.authenticated_root}) thread_ctx);
    Tgroup.ThreadGroup.(add tg_cli ; add tg_authenticated_root)
  in
  let free () = Tgroup.ThreadGroup.destroy ();
    Xapi_stdext_threads.Threadext.ThreadRuntimeContext.remove () in
  Test.make_with_resource ~name ~allocate ~free Test.uniq f

let benchmarks =
  Test.make_grouped ~name:"timeslice"
    [
      test_with_thread_classified ~name:"maybe_thread_yield" (Staged.stage test_maybe_thread_yield)
