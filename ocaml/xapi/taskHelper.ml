...
let compare_auth_user_ids_of_sessions ~__context s1 s2 =
  Context.with_tracing ~__context __FUNCTION__ @@ fun __context__ ->
  ...
  | Some context_session ->
      let is_own_task =
        ...
        with e ->
          ...
          debug "current_session: %s" (Context.trackid __context) ;
          debug "given task id : %s" (Ref.really_pretty_and_small task_id) ;
          debug "context task id : %s"
            (Ref.really_pretty_and_small (Context.get_task_id __context)) ;
          false
      in
